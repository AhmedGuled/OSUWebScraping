...
file_path = './directories/WGSS_directory.csv'
...
for index, row in professors_data.iterrows():
    ...
    url = f"https://wgss.osu.edu/people/{lastname}.{dot_num}"
...
file_exists = os.path.isfile("./TechDirectories/Tech_Interested_Professors_WGSS.csv")

if file_exists:
    output_df.to_csv("./TechDirectories/Tech_Interested_Professors_WGSS.csv", mode='a', header=False, index=False)
else:
    output_df.to_csv("./TechDirectories/Tech_Interested_Professors_WGSS.csv", index=False)

print("Script complete. Results appended to Tech_Interested_Professors_WGSS.csv.")
