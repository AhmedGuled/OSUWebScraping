...
for index, row in professors_data.iterrows():
    ...
    url = f"https://fisher.osu.edu/people/{lastname}.{dot_num}"  # Fixed URL construction
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            specific_div = soup.find('div', class_="bux-container")
            if specific_div:
...
file_exists = os.path.isfile("./TechDirectories/Tech_Interested_Professors_fisher.csv")

# Append results to the CSV file
if file_exists:
    # If the file exists, append without writing the header
    output_df.to_csv("./TechDirectories/Tech_Interested_Professors_fisher.csv", mode='a', header=False, index=False)
else:
    # If the file doesn't exist, create it with headers
    output_df.to_csv("./TechDirectories/Tech_Interested_Professors_fisher.csv", index=False)

print("Script complete. Results appended to Tech_Interested_Professors_fisher.csv.")
